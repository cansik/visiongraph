...
class LandmarkDetectionResult(ObjectDetectionResult):
    ...
    def __init__(
        ...
        class_name: str,
        score: float,
        landmarks: vector.VectorNumpy4D,
    ...
    def annotate(
        ...
        stroke_width: int = 2,
        landmark_colors: Optional[Union[Sequence[int], Sequence[Sequence[int]]]] = None,
        **kwargs,
    ):
        """
    ...
    def map_coordinates(
        ...
        self.landmarks.x[:] = (
            (((self.landmarks.x * src_width) - src_roi.x_min) / src_roi.width) * dest_roi.width + dest_roi.x_min
        ) / dest_width
        self.landmarks.y[:] = (
            (((self.landmarks.y * src_height) - src_roi.y_min) / src_roi.height) * dest_roi.height + dest_roi.y_min
        ) / dest_height
