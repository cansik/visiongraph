...
class AdasHeadPoseEstimator(FaceLandmarkEstimator):
    ...
    def estimate(self, image: np.ndarray, **kwargs) -> FaceLandmarkResult:
        ...
        return FaceLandmarkResult(1.0, list_of_vector4D(result))

    def _transform_result(self, result: FaceLandmarkResult, image: np.ndarray, roi: np.ndarray, xs: float, ys: float):
        hi, wi = image.shape[:2]
        hr, wr = roi.shape[:2]

        for lm in result.landmarks:
            lm.x = ((lm.x * wr) + xs) / float(wi)
            lm.y = ((lm.y * hr) + ys) / float(hi)
