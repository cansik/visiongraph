...
from visiongraph.util.VectorUtils import lerp_vector_3d, lerp_vector_2d, vector_distance
...
def _get_pose_distance_embedding(pose: PoseLandmarkResult) -> np.ndarray:
    ...
    embedding = np.array(
        [
            ...
            # Body bent direction.
            vector_distance(
                lerp_vector_3d(pose.left_wrist.to_xyz(), pose.left_ankle.to_xyz(), 0.5), pose.left_hip.to_xyz()
            ),
            vector_distance(
                lerp_vector_3d(pose.right_wrist.to_xyz(), pose.right_ankle.to_xyz(), 0.5), pose.right_hip.to_xyz()
            ),
        ]
    )
